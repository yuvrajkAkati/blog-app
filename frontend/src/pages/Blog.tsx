
import { Topbar } from "../components/Topbar"

interface Blog {
    title : string,
    description : string,
    publishedDate : string,
    minutesRead : number
    author : {
        name : string
    }

}
export const Blog = ({blog}:{blog : Blog})=>{
    return <div>
        <Topbar></Topbar>   
        
        
        <div className="px-96 pt-10">
        <div className="flex justify-center font-bold text-5xl pb-10 divide-x">
            Lorem ipsum dolor sit sad sad asd.
        </div>
        <div className="flex pb-16  flex justify-">
            <div className="bg-red-500 grid place-items-center rounded-full px-5">Y</div>
            <div className="pl-4">
                <div>
                    yuvraj
                </div>
                <div>
                    7 min read . May 28,2024
                </div>
            </div>
        </div>
        <div className=" flex justify-center">
            <ImageProvider></ImageProvider>
        </div>
        <div className="text-2xl text-center pt-10 pb-10">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Natus magni optio veritatis veniam architecto ex quam eum animi suscipit ea distinctio alias sed ad itaque at, eos blanditiis similique perferendis error ut laudantium maiores doloremque impedit. Nesciunt nobis harum ea et, ipsum minus perspiciatis ut alias culpa expedita ipsam cumque ex saepe architecto iusto dicta. Incidunt, 
            
           <p> dolores quod aliquid hic omnis in voluptas saepe recusandae eos commodi? Debitis cumque saepe velit, reprehenderit nisi deserunt necessitatibus est repudiandae porro id, a corrupti nesciunt! Minima repudiandae delectus molestias. Earum rerum, quasi cupiditate unde, et nulla voluptas quidem aut sint accusantium sequi sed praesentium perspiciatis molestias! Tempora numquam nesciunt eum quos? Accusamus laborum laudantium deserunt! Laboriosam ipsam atque harum impedit alias voluptates, modi molestias dolorem, iure eos expedita adipisci delectus dolorum tenetur et cupiditate eveniet eligendi. Animi ab omnis et corrupti dolores excepturi dolorum! Error, ipsum! Quia numquam minima quam sed officia at, veritatis voluptatibus accusamus eaque culpa quos libero in ad nesciunt nihil, mollitia deserunt quibusdam odit doloremque tenetur soluta dolores? Nulla soluta consequatur ipsum ullam, expedi</p>
           ta illum maxime labore, dignissimos mollitia placeat nobis nostrum repellendus. Dolores, repudiandae dolorem? Quibusdam unde molestiae dolores ex, iure facilis deleniti aliquid, dicta excepturi, facere vel!
        </div>
        </div>
        <div className="bg-blue-50 h-max">
            <div className="px-96">
            
            <div className="flex pt-40">
                <div className="text-slate-900 shadow-md bg-white border rounded-full px-9 flex place-items-center py-6"><div className='text-2xl'>y</div></div>
            </div>

            <div className="flex justify-between">
                <div className="pt-5 pl-2   place-items-center"><div className="text-2xl">Written By Authorname</div>
                <div className="text-slate-500 pt-3">followers 77</div>
                </div>
                <div className="flex  place-items-center">
                <div className=""><button className="bg-green-600 p-2 px-3 rounded-lg text-white">Follow</button></div><div className="pl-3"><button className="bg-green-600 text-white rounded-full p-2 px-2 ">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
                </svg>
                </button></div>
                </div>
            </div>    
            
            <hr className="h-px my-8 bg-gray-200 border-0 dark:bg-gray-300"></hr>
            
            <div className=" pt-24 text-xl text-slate-600 font-bold">
                More from Authorname
            </div>     

            <div className="">
                <div className="pt-8 grid grid-cols-2">
                <div className="py-4 px-4"><MiniBlogCard></MiniBlogCard></div>
                <div className="py-4 px-4"><MiniBlogCard></MiniBlogCard></div>
                <div className="py-4 px-4"><MiniBlogCard></MiniBlogCard></div>
                <div className="py-4 px-4"><MiniBlogCard></MiniBlogCard></div>
                </div>
            </div>   

            </div>
        </div>

    </div>
}




function MiniBlogCard({blog}:{blog : Blog}){
    return (
        
            <div className="">
                <div className="grid grid-rows-2 h-96">
                    <div className="flex justify-center bg-white "><ImageProvider/></div>
                    <div className="flex flex-col justify-between bg-blue-50">
                       <div>
                            <div className="pt-3 flex pl-1" >
                                <div className="pl-3 pr-3 pt-1 pb-1 rounded-full bg-slate-200">y</div>
                                <div className="text-sm grid grid-cols place-items-center"><div className="pl-1">authorName</div></div>
                            </div>
                            <div className="line-clamp-2 pl-1 pt-1 text-xl font-bold">20 CSS One-Liners Every CSS Expert Needs to Know</div>
                            <div className="line-clamp-2 text-slate-500 pl-1 pt-1">There are many other examples pushing the limits of HTML and CSS including interactive carousels, accordions, calculating, counting, advanced input validation, state management, dismissible modal windows, and reacting to mouse and keyboard inputs. There’s even an animated pure CSS clock (analog and digital) and a fully working star rating widget!</div>
                        </div>
                        <div className="pb-2 px-1 flex justify-between text-slate-350">
                            <div className="">
                                26 may
                            </div>
                            <div className="flex">
                                <div className="flex place-items-center"><button><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
                                </svg></button></div>
                                <div className="pl-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 12a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM12.75 12a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM18.75 12a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                                    </svg>

                                </div>
                            </div>
                        </div> 
                       
                    </div>
                </div>
            </div>
    )
    
}






function ImageProvider(){
    return (
           
                        <img  className='h-96 w-96 rounded-lg' src="data:image/jpeg;base64," alt="rust cohle" />
                  
    
    )
}